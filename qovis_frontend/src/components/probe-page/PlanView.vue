<template>
  <g class="plan-view" :transform="`translate(${[plan.x, plan.y]})`">
    <g class="plan-group"
       :transform="'translate(' + [planPadding, planPadding] + ')'">
      <g class="edge-group">
        <g v-for="(edge, i) in plan.edges" :key="i">
...
<!--            <title>{{ edge.planEdgeStr }}</title>-->
      ...
      <g class="node-group">
        <g v-for="(node, i) in plan.nodes" :key="i" @click="clickNode(node)"
...
import {VisQueryPlan} from "@/js/probe/VisQueryPlan"
...
export default {
  ...
  methods: {
    ...
    /**
     * @param {import("@/js/probe/VisQueryPlan").VisPlanNode} visPlanNode
     */
    getNodeNameWithId(visPlanNode) {
      ...
      if (useOptAlias) {
        name = this.optInfoMap.get(visPlanNode.name)?.alias ?? '?'
      }
  ...
  computed: {
    ...
    width() {
      return this.plan?.width ?? 0
    },
    height() {
      return this.plan?.height ?? 0
    },
  }
}
</script>
