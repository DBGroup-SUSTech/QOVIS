...
export const QuestionList = [
  new Question(
      ...
      [
        ...
        {id: 1, text: 'B. Consists of **transformation issue**. A wrong operator *Filter* is added during the *Optimization phase*.'},
        {id: 2, text: 'C. Consists of **transformation issue**. A wrong expression on the operator *Project* is generated during the *Optimization phase*.'},
  ...
  new Question(
      ...
      [
        ...
        {id: 2, text: 'C. Consists of **transformation issue**. A wrong predicate is added to the *InnerJoin* during the *Optimization phase*.'},
  ...
  new Question(
      ...
      [
        ...
        {id: 2, text: 'C. Consists of **transformation issue**. One *InnerJoin* predicate on optimized plan is not derived from the predicates in the *Filter* of analyzed plan.'},
  ...
  new Question(
      ...
      [
        ...
        {id: 3, text: 'D. Consists of **workflow issue**. The optimizer does not conduct *DeduplicateRelations* during the *Analysis phase*.'},
      ],
      2
  ),
  new Question(
  ...
  new Question(
      ...
      '![Case 0](figures/issue1_query.png =*x100)' +
      '</br></br>' +
      'This case consists of a **workflow issue**. A part of optimization steps repeats multiple times (100 times) and reaches the max iteration threshot.' +
      ...
      [
        ...
        {id: 1, text: 'B. *ColumnPruning* step in the previous loop and *CollapseProject* in the next loop generate/consume two *Project* nodes, so they cancel each other out and form a loop.'},
        {id: 2, text: 'C. *PushDownLeftSemiAntiJoin* step does not push down the *leftAntiJoin* successfully so that the optimizer try to optimize it again.'},
        {id: 3, text: 'D. *ColumnPruning* step and *RemoveNoopOperators* step add/remove a *Project* node so that the optimizer continue optimizing.'},
      ],
      1
  ),
  new Question(
      ...
      [
        ...
        {id: 3, text: 'D. *ResolveJoinStrategyHints* step in *Analysis phase* removes the hint node unexpectedly, so the executor chooses to broadcast table *s*.'},
      ],
      3
  ),
  new Question(
      ...
      '</br></br>' +
      'Tips: Try to track the **join prediction** forwards or backwords.',
      'The root cause is that **in Optimization phase**:',
      [
        {id: 0, text: 'A. The join predicate is rewritten incorrectly by *PushDownPredicates* step, then the optimizer propagates it and results in an empty selection.'},
        {id: 1, text: 'B. The join predicate is rewritten incorrectly by *InferFiltersFromConstraints* step, then the optimizer propagates it and results in an empty selection.'},
        {id: 2, text: 'C. The join predicate is rewritten incorrectly by *PushdownLeftSemiAntiJoin* step, then the optimizer propagates it and results in an empty selection.'},
