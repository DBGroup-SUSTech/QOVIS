...
export class PatternTraceTree2 {
  ...
  constructor(config, dg, patterns) {
    ...
    for (let i = 0; i < matchGroups.length; i++) {
      ...
      for (let j = 0; j < group.length; j++) {
        ...
        phaseNode.init(this, phasePatternNode, phaseName, start === 0 ? 0 : start + 1, end)
    ...
    for (const phasePatternNode of this.root.children) {
      for (const phaseNode of phasePatternNode.children) {
        const phasePlans = phaseNode.getPlans()
        ...
        for (const batchPatternNode of phaseNode.children) {
          for (const batchNode of batchPatternNode.children) {
            ...
            assert(startIdx2 === batchNode.endIdx, 'batch plan check failed')
          }
        }
  ...
  inferBatchName(batchPlans) {
    ...
    throw new Error("Cannot infer batch name")
  }

  simplifyTree() {
    const simplifyNode = (node) => {
      if (node.children.length === 1 && node.isPatternNode()) {
        assert(node.parent !== null)
        const parent = node.parent
        const child = node.children[0]
        parent.children.splice(parent.children.indexOf(node), 1, child)
        child.parent = parent
        simplifyNode(child)
      } else {
    ...
    simplifyNode(this.root)

    // update level
    const updateLevel = (node, level) => {
      node.level = level
      node.children.forEach(child => updateLevel(child, level + 1))
    }
    updateLevel(this.root, 0)
  }
  ...
  initPlanBlocks(config) {
    ...
    for (let i = 1; i < this.planBlocks.length - 1; i++) {
      ...
      planBlock.height = rowHeight * (this.maxLevel - ancestor.level) + blockExtraHeight
    }
  ...
  /**
   * @param {PatternTraceNode2} leftNode
   * @param {PatternTraceNode2} rightNode
   * @returns {PatternTraceNode2}
   */
...
export class PatternTraceNode2 {
   ...
   */
  get endVPlanDag() {
    return this.traceTree.getVPlanDag(this.endIdx - 1)
  }

  getPlans() {
    return this.traceTree.plans.slice(this.startIdx, this.endIdx)
  }
  ...
  initialLayout(config, x) {
    ...
    if (this.expanded) {
      ...
      if (['Replace Operators', 'Operator Optimization before Inferring Filters', 'PushDownLeftSemiAntiJoin'].includes(this.getSimpleName())) {
        this.width += 70
      }
    }
  }
