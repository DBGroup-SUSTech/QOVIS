...
export class PatternTraceTree2 {
  ...
  constructor(config, dg, patterns) {
    ...
    const matchGroups = this.matchPattern(phaseNames)
    ...
    for (let i = 0; i < matchGroups.length; i++) {
      ...
      for (let j = 0; j < group.length; j++) {
        ...
        phaseNode.init(this, phasePatternNode, phaseName, start + 1, end)
    ...
    for (const phasePatternNode of this.root.children) {
      for (const phaseNode of phasePatternNode.children) {
        const phaseMetas = phaseNode.getPlans().slice(1).map(plan => plan.meta)

        // split phase plans to batch

        const batchMetasList = []
        let batchMetas = [phaseMetas[0]]
        const getBatchId = (index) => {
          const meta = phaseMetas[index]
          if (meta.batchId == null && index < phaseMetas.length - 1) {
            return getBatchId(index + 1)
          } else {
            return meta.batchId ?? ''
          }
        }
        for (let i = 1; i < phaseMetas.length; i++) {
          const meta = phaseMetas[i]
          if (getBatchId(i - 1) === getBatchId(i)) {
            batchMetas.push(meta)
          } else {
            batchMetasList.push(batchMetas)
            batchMetas = [meta]
          }
        }
        ...
        const batchNames = batchMetasList.map(metas => this.inferBatchName(metas))
        const matchGroups = this.matchPattern(batchNames)

        let startIdx1 = phaseNode.startIdx
        let batchMetasListPtr = 0
        for (let i = 0; i < matchGroups.length; i++) {
          const batchPatternNode = new PatternTraceNode2(NodeType.BATCH_PATTERN)
          const group = matchGroups[i]
          const metaNum = batchMetasList.slice(batchMetasListPtr, batchMetasListPtr + group.length)
              .reduce((acc, cur) => acc + cur.length, 0)
          batchPatternNode.init(this, phaseNode, group[0], startIdx1, startIdx1 + metaNum)
          phaseNode.children.push(batchPatternNode)

          for (let j = 0; j < group.length; j++) {
            const batchMetas = batchMetasList[batchMetasListPtr + j]
            const batchName = group[j]    // in fact, each element in group is the same

            const batchNode = new PatternTraceNode2(NodeType.BATCH)
            batchNode.init(this, batchPatternNode, batchName, startIdx1, startIdx1 + batchMetas.length)
            batchPatternNode.children.push(batchNode)

            startIdx1 += batchMetas.length
          }
          batchMetasListPtr += group.length
        }
        ...
        //     const batchPlans = batchNode.getPlans()
        //     const actionNames = batchPlans.map(p => p.meta.getSimpleName())
        //
        //     const matchGroups = this.matchPattern(actionNames)
        //
