...
class PrioritizedItem:
    ...
    f: int = field(compare=False)
    hu: int = field(compare=False)

    def __init__(self, plan: QueryPlan, f: int, hu: int):
        self.priority = (f << 10) + hu
        self.plan = plan
        self.f = f
        self.hu = hu


class MinStepExtractor2:
    ...
    def hard_apply(self, rule_seq: list[Rule], match_index_list: list[Optional[int]] = None) -> list[QueryPlan]:
        cur_level: list[QueryPlan] = [self.t_src]
        mem = {self.t_src.to_hash_str()}
        for i, rule in enumerate(rule_seq):
            print('path', rule_seq[:i+1], 'plans', len(cur_level))
            if len(cur_level) == 0:
                raise Exception('no plan to apply rule')
            next_level: list[QueryPlan] = []
            for t in cur_level:
                # apply the rule
                if match_index_list is not None and match_index_list[i] is not None:
                    m_idx = match_index_list[i]
    ...
    def execute(self) -> Optional[list[Rule]]:
        ...
        src_hu = self.get_delta_dist(self.t_src, self.t_dst)
        ...
        cand_open_q: PriorityQueue[PrioritizedItem] = PriorityQueue()

        path_dict[self.t_src] = []
        open_q.put(PrioritizedItem(self.t_src, src_f, src_hu))
        open_set[self.t_src] = None

        # test
        # tmp_src = self.hard_apply([ReorderInnerJoin] * 3, [0, 1, 2])[0]
        # # assert tmp_src.has_same_struct(self.t_dst)
        # path_dict[tmp_src] = []
        # open_q.put(PrioritizedItem(tmp_src, src_f, src_hu))
        # open_set[tmp_src] = None
        ...
        while True:
            from_open = False
            if open_q.empty():
                # try to use cand_open_q
                if cand_open_q.empty():
                    # no more candidates
                    break
                else:
                    item = cand_open_q.get()
            else:
                item = open_q.queue[0]
                if cand_open_q.qsize() > 0 and cand_open_q.queue[0].priority <= item.priority:
                    item = cand_open_q.get()
                else:
                    from_open = True
                    item = open_q.get()
            ...
            for rule in self.rules:
                ...
                for new_t in new_ts:
                    if new_t.semantically_equals(self.t_dst):
                        assert new_t.has_same_struct(self.t_dst)
                        path = t_path + [rule]
                    ...
                    new_hu = self.get_delta_dist(new_t, self.t_dst)
                    if new_t not in open_set:
                        h = self.get_min_step_cnt(new_t, self.t_dst)
                        f = g + h
                        if new_hu < hu:
                            # add to open set
                            open_q.put(PrioritizedItem(new_t, f, new_hu))
                            open_set[new_t] = None

                            if rand.random() < self.sample_rate:
                                self.append_result(g, h, new_t, path_dict[new_t])
                        else:
                            # add to candidate open set
                            cand_open_q.put(PrioritizedItem(new_t, f, new_hu))
    ...
    #     step_cnt = 0
    #     for d, max_d in zip(delta, self.max_rule_delta):
    #         if max_d == 0:
    #             continue
    #         step_cnt = max(step_cnt, d / max_d)
    #     return ceil(step_cnt)
