...
class SerializeFromObjectExec(PlanNode):
    def __init__(self):
        super().__init__()
    ...
    def complete_param_and_check(self):
        ...
        attr_str_list = AttrPattern.findall(self.str_)
    ...
    def raw_cols(self) -> list[str]:
        s0 = self.str_[self.str_.find('[') + 1: self.str_.rfind(']')]
    ...
    def dump_params(self) -> list[tuple[str, list]]:
        return [('serializer', self.raw_cols)]

    def get_param_name(self, param: Any) -> Optional[str]:
        if param is self.raw_cols:
            return "serializer"
        return None
