...
def build_insert_tree(tree: PlanNode) -> AlignNode:
    if tree is None:
        return None
    return AlignNode.build_with_children(
        Transform(None, tree.vid, TransType.INSERT),
        [build_insert_tree(c) for c in tree.providers]
    )


def build_delete_tree(tree: PlanNode) -> AlignNode:
    if tree is None:
        return None
    return AlignNode.build_with_children(
        Transform(tree.vid, None, TransType.DELETE),
        [build_delete_tree(c) for c in tree.providers]
    )


def build_transfer(u: Optional[PlanNode], v: Optional[PlanNode]) -> Transform:
...
class TreeAlignAlgo:
    ...
    def exec(self):
        ...
        for i in range(1, len(nodes1)):
            ...
            self.forest1_to_empty_res[i] = [self.get_tree_res_mem(self.nodes1.index(c), 0) for c in u.providers]
        ...
        for j in range(1, len(nodes2)):
            ...
            self.empty_to_forest2_res[j] = [self.get_tree_res_mem(0, self.nodes2.index(c)) for c in v.providers]
    ...
    def compute_dist_tree(self, i, j):
        ...
        if dist == case1:
            # find w
            w = find(v.providers, lambda w: self.get_tree_mem(i, self.nodes2.index(w)) - self.get_tree_mem(0, self.nodes2.index(w)) == dist - self.get_tree_mem(0, j))
            assert w is not None
            child_aligns = [self.get_tree_res_mem(i, self.nodes2.index(x)) if x == w    # u w
                            else build_insert_tree(x)       # \lambda x
                            for x in v.providers]
            result = AlignNode.build_with_children(Transform(None, v.vid, TransType.INSERT), child_aligns)  # \lambda v

        elif dist == case2:
            # find w
            w = find(u.providers, lambda w: self.get_tree_mem(self.nodes1.index(w), j) - self.get_tree_mem(self.nodes1.index(w), 0) == dist - self.get_tree_mem(i, 0))
            assert w is not None
            child_aligns = [self.get_tree_res_mem(self.nodes1.index(x), j) if x == w    # w v
                            else build_delete_tree(x)       # x \lambda
                            for x in u.providers]
